...
export async function applySchema(): Promise<void> {
  ...
  const safeQueries: string[] = [
    ...
    `CREATE TABLE IF NOT EXISTS hotel_rooms (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), hotel_id UUID REFERENCES hotels(id) ON DELETE CASCADE, name VARCHAR(255) NOT NULL, description TEXT, size_sqft INTEGER, view VARCHAR(100), capacity INTEGER, base_rate NUMERIC(10,2), images JSONB, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`,
    `CREATE TABLE IF NOT EXISTS hotel_venues (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), hotel_id UUID REFERENCES hotels(id) ON DELETE CASCADE, name VARCHAR(255) NOT NULL, description TEXT, sqft INTEGER, ceiling_height_ft DECIMAL(5,2), capacity_reception INTEGER, capacity_banquet INTEGER, capacity_theater INTEGER, outdoor BOOLEAN DEFAULT false, details JSONB, images JSONB, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`,
    ...
    `CREATE TABLE IF NOT EXISTS hotel_dining (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), hotel_id UUID REFERENCES hotels(id) ON DELETE CASCADE, name VARCHAR(255) NOT NULL, cuisine VARCHAR(100), description TEXT, hours VARCHAR(255), dress_code VARCHAR(100), details JSONB, images JSONB, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`,
